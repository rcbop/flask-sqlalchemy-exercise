...
def test_get_item(test_client, db_fixture):
...
def test_delete_item(test_client, db_fixture):
    item = ItemModel(name="test item", price=9.99, store_id=1)
    db_fixture.session.add(item)
    db_fixture.session.commit()
...
def test_put_item(test_client, db_fixture):
    item = ItemModel(name="test item", price=9.99, store_id=1)
    db_fixture.session.add(item)
    db_fixture.session.commit()
...
def test_get_item_list(test_client, db_fixture):
...
def test_post_item(test_client):
