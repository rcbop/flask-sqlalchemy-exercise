...
def user_fixture(db_fixture):
    user = UserModel(
        username="test", password=pbkdf2_sha256.hash("test"), email="john@doe.com"
    )
...
def test_logout_user(test_client, tokens):
    ...
    response = test_client.get(
        "/user/1", headers={"Authorization": f"Bearer {tokens[0]}"}
    )
    assert response.status_code == 401
...
def test_refresh_token(test_client, tokens):
    response = test_client.post(
        "/refresh", headers={"Authorization": f"Bearer {tokens[1]}"}
    )
    assert response.status_code == 200
    assert "access_token" in response.json
